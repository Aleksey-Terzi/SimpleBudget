...
namespace SimpleBudget.API.Controllers
{
    ...
    public class TaxesController : BaseApiController
    {
        ...
        {
            return await _taxService.CreateModelAsync(AccountId, personId, year, null, null);
        }

        [HttpPost("close")]
        public async Task<ActionResult<TaxModel>> CloseYear(int? personId, int? year)
        {
            return await _taxService.CloseYearAsync(AccountId, personId, year);
        }

        [HttpPost("open")]
        public async Task<ActionResult<TaxModel>> OpenYear(int? personId, int? year)
        {
            return await _taxService.OpenYearAsync(AccountId, personId, year);
        }
    }
}
