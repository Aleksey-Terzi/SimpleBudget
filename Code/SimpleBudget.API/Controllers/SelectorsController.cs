...
namespace SimpleBudget.API.Controllers
{
    ...
    public class SelectorsController : BaseApiController
    {
        ...
        public async Task<ActionResult<SelectorsModel>> GetSelectors()
        {
            ...
            model.Companies = (await _companySearch.Bind(x => x.Name, x => x.AccountId == AccountId))
                .Distinct()
                .OrderBy(x => x)
                .ToList();

            model.Categories = (await _categorySearch.Bind(x => x.Name, x => x.AccountId == AccountId))
                .Distinct()
                .OrderBy(x => x)
                .ToList();

            model.Wallets = (await _walletSearch.Bind(x => x.Name, x => x.AccountId == AccountId))
                .OrderBy(x => x)
                .ToList();

            model.Persons = (await _personSearch.Bind(x => x.Name, x => x.AccountId == AccountId))
                .OrderBy(x => x)
                .ToList();
